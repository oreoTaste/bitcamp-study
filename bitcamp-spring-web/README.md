...
- /WEB-INF/app-context.xml 파일 생성
  - Spring IoC 컨테이너 설정 파일 (DispatcherServlet 역할임ㅇㅇ)
...
- /WEB-INF/web.xml 변경
  ...
  - /WEB/INF/서블릿이름-servlet.xml 파일을 기본으로 찾는다. (없으면 ClassNotFound 예외발생)
- 만약, DispatcherServlet에서 IoC컨테이너를 설정하기 싫다면, contextConfigLocation의 값을 빈채로 두어라.
  (즉, 설정을 안하는것은 기본설정이고, 설정을 하는데 비워두는건 안한다는 뜻이다.)
