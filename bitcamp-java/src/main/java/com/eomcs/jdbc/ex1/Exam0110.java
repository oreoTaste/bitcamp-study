...
public class Exam0110 {
  ...
  public static void main(String[] args) {
    ...
    // Driver 구현체는 DBMS에 연결작업을 수행한다.
    // Driver 구현체는 DriverManager가 관리한다.
    // => 따라서, 접속할 DBMS의 Driver 구현체를 생성하여 DriverManager에게 등록해야한다.
    
    // Driver Manager (java.sql.Driver의 구현객체를 관리함)
    // DBMS 연결 요청이 들어오면, 해당 DBMS의 Driver 구현체를 찾아 작업을 위임하낟.
    ...
    try {
      ...
      // DriverManager에 등록된 인스턴스 Driver를 확인해보자!
      java.sql.Driver driver = DriverManager.getDriver("jdbc:mariadb:");
      System.out.println(driver);

      // 등록되지 않은 드라이버를 찾을 경우 예외발생
      java.sql.Driver driver2 = DriverManager.getDriver("jdbc:oracle:");
