		...
		<form action='update' method='post' enctype='multipart/form-data'>
			...
			<p>
				<c:forEach items="${photoBoard.files}" var="photoFile">
					<img
						src='${pageContext.servletContext.contextPath}/upload/photoboard/${photoFile.filePath}'
						height='80'/>
				</c:forEach>
			</p>

			사진: <input name='photoFiles' type='file'/><br> 사진: <input
				name='photoFiles' type='file'/><br> 사진: <input
				name='photoFiles' type='file'/><br> 사진: <input
				name='photoFiles' type='file'/><br> 사진: <input
				name='photoFiles' type='file'/><br>
			<p>
				<button>변경</button>
				<a
					href='delete?no=${photoBoard.no}&lessonNo=${photoBoard.lesson.no}'>삭제</a>
			</p>
	</form>
	</c:if>
